...
def generate_commentary(item):
    ...
    prompt = (
        f"You are an inventory analyst. Given the item: {item}, "
        "write a concise 2-3 sentence recommendation explaining why to reorder "
        "and any risks."
    )

    resp = client.chat.completions.create(
        model="gpt-4o",  # Or gpt-4o-mini if you want faster/cheaper
...
if __name__ == "__main__":
    ...
    df = pd.read_csv(
        os.path.abspath(
            os.path.join(os.path.dirname(__file__), '..', 'output', 'draft_pos.csv')
        )
    )
